...
from main.firebase_config import get_firebase_database_ref, upload_file_to_database

class Command(BaseCommand):
    help = 'Migrate files from local storage to Firebase Realtime Database'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--destination-dir',
            default='applogo',
            help='Directory in Firebase Realtime Database to migrate files to (default: applogo)'
        )
        parser.add_argument(
            '--delete-local',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if not dry_run:
            db_ref = get_firebase_database_ref()
            if not db_ref:
                self.stdout.write(self.style.ERROR('Failed to get Firebase Realtime Database reference'))
                self.stdout.write(self.style.WARNING('Check your Firebase credentials and database configuration'))
                self.stdout.write(self.style.WARNING('Make sure the Realtime Database exists and the service account has access to it'))
                return
        else:
        ...
        for filename in files:
            ...
            self.stdout.write(f'Migrating {filename} to Firebase Realtime Database...')
            ...
            try:
                if not dry_run:
                    ...
                    media_url = upload_file_to_database(file_path, firebase_path)
                    
                    if media_url:
                        self.stdout.write(self.style.SUCCESS(f'Successfully uploaded {filename} to Firebase Realtime Database'))
                        ...
                        self.stdout.write(self.style.ERROR(f'Failed to upload {filename} to Firebase Realtime Database'))
                else:
                    # Dry run - just log what would happen
                    self.stdout.write(f'[DRY RUN] Would upload {filename} to Firebase Realtime Database at path: {firebase_path}')
                    success_count += 1
